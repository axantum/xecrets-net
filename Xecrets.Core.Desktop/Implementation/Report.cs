...
internal class Report(string reportFileName, long maxReportFileLength) : IReport
{
    ...
    public void Exception(Exception ex)
    {
        ...
        sb.Append(FormattableString.Invariant($"----------- Exception at {New<INow>().Utc:u} -----------")).AppendLine();
    ...
    private static readonly string RsaDecryptionStackMarker = $"at {typeof(NetAsymmetricFactory).FullName!.Replace(nameof(NetAsymmetricFactory), "NetPrivateKey")}.{nameof(IAsymmetricPrivateKey.Transform)}";

    private static bool IgnoreException(Exception ex)
    {
        // Ignore simple failures to decrypt a private key
        if (ex is CryptographicException { HResult: unchecked((int)0xc100000d) })
        {
            return true;
        }

        if ((ex.StackTrace ?? string.Empty).Contains(RsaDecryptionStackMarker))
        {
            return true;
        }

...
namespace Xecrets.Core.Desktop.Abstractions;

public interface IDesktopServices
{
    IFile StandardIoFile(string path);

    IFile File(string path);

    IFile FindFree(string fullPath, bool overwrite);

    bool CanReadFromFile(IFile file, [NotNullWhen(false)] out string? reason);

    bool CanWriteToFile(IFile file);

    bool CanWriteToFolder(IFolder folder);

    Task<bool> WipeAsync(string path, IProgress<Progress> progress);
}

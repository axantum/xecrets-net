    ...
    public class FileLock : IDisposable
    {
        ...
        public static FileLock Lock(IRuntimeFileInfo fileInfo)
        {
            ...
            lock (_lockedFiles)
            {
                ...
                {
                    Os.Log.Info("Locking file '{0}'.".InvariantFormat(fileInfo.FullName));
                }
            ...
            foreach (IRuntimeFileInfo fileInfo in fileInfoParameters)
            {
                ...
                lock (_lockedFiles)
                {
                    if (_lockedFiles.Contains(fileInfo.FullName))
                    {
                        if (Os.Log.IsInfoEnabled)
                        {
                            Os.Log.Info("File '{0}' was found to be locked.".InvariantFormat(fileInfo.FullName));
                        }
                        return true;
                    }
                }
        ...
        protected virtual void Dispose(bool disposing)
        {
            lock (_lockedFiles)
            {
                ...
                if (Os.Log.IsInfoEnabled)
                {
                    Os.Log.Info("Unlocking file '{0}'.".InvariantFormat(_fullPath));
                }
                _fullPath = null;
            }
        }
